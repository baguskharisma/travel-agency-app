<nav class="items-center justify-between hidden h-24 px-16 pt-20 mx-auto md:flex">
    <div class="flex items-center">
        <div class="flex items-center justify-center w-32 h-24 mr-4 border-b-4 border-transparent">
            <svg class="mr-2 fill-white" width="24" height="20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M8.747 19.75H7.5a.75.75 0 0 1-.683-1.06l3.021-6.667-4.537-.101-1.655 2.005c-.315.396-.567.573-1.209.573h-.84a.828.828 0 0 1-.689-.33c-.112-.151-.221-.406-.114-.77l.929-3.328a.857.857 0 0 1 .024-.074V9.99a.695.695 0 0 1-.024-.075l-.93-3.35c-.101-.356.009-.605.12-.752A.789.789 0 0 1 1.55 5.5h.887c.48 0 .945.215 1.219.563l1.62 1.97 4.562-.067-3.02-6.656A.75.75 0 0 1 7.5.25h1.26a1.172 1.172 0 0 1 .908.46l5.862 7.124 2.708-.07c.198-.012.747-.015.874-.015 2.59.001 4.138.842 4.138 2.251 0 .443-.178 1.266-1.363 1.789-.7.31-1.634.466-2.776.466-.125 0-.673-.004-.874-.015l-2.708-.072-5.876 7.125a1.174 1.174 0 0 1-.906.457Z" />
            </svg>
            <a href="#" class="font-semibold text-white text-md font-montserrat">Find
                Flight</a>
        </div>
        <div class="flex items-center justify-center w-32 h-24 border-b-4 border-transparent">
            <svg class="mr-2 fill-white" width="22" height="18" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M19.25 7.814a3.724 3.724 0 0 0-1.5-.314H4.25A3.754 3.754 0 0 0 .5 11.25v5.25a.75.75 0 1 0 1.5 0v-.375a.38.38 0 0 1 .375-.375h17.25a.38.38 0 0 1 .375.375v.375a.75.75 0 1 0 1.5 0v-5.25a3.754 3.754 0 0 0-2.25-3.436ZM16.625.75H5.375A2.625 2.625 0 0 0 2.75 3.375V6.75a.187.187 0 0 0 .24.18c.409-.12.833-.18 1.26-.18h.198a.188.188 0 0 0 .188-.166A1.5 1.5 0 0 1 6.125 5.25H8.75a1.5 1.5 0 0 1 1.49 1.334.188.188 0 0 0 .188.166h1.147a.187.187 0 0 0 .187-.166A1.5 1.5 0 0 1 13.25 5.25h2.625a1.5 1.5 0 0 1 1.49 1.334.188.188 0 0 0 .188.166h.197c.427 0 .851.06 1.26.18a.188.188 0 0 0 .24-.18V3.375A2.625 2.625 0 0 0 16.625.75Z" />
            </svg>
            <a href="#" class="font-semibold text-white text-md font-montserrat">Find Stays</a>
        </div>
    </div>
    <svg class="w-28 h-9" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g clip-path="url(#clip0_108_21029)">
            <path
                d="m14.728 5.577 3.219 2.431-1.966 2.502c1.357 1.536 1.857 3.288 1.857 5.22 0 2.18-.822 5.254-3.719 6.577 2.932 1.466 3.646 3.575 3.646 5.828 0 4.863-3.719 7.865-8.83 7.865C3.824 36 0 32.89 0 28.135h4.324c0 2.288 2.11 3.79 4.611 3.79 2.502 0 4.468-1.358 4.468-3.79 0-2.431-2.288-3.54-4.468-3.54C3.432 24.595 0 21.236 0 15.73c0-5.507 4.002-8.94 8.938-8.94 1.396 0 2.823.18 4.04 1.001l1.75-2.214ZM4.324 15.729c0 3.076 2.075 4.898 4.611 4.898 2.502 0 4.576-1.857 4.576-4.897S11.44 10.76 8.938 10.76c-2.54 0-4.614 1.893-4.614 4.969ZM50.567 0v24.99h-4.324V0h4.324ZM69.999 16.195c0 5.076-3.467 9.187-9.187 9.187-5.72 0-9.152-4.11-9.152-9.187 0-5.042 3.502-9.187 9.117-9.187 5.614 0 9.222 4.145 9.222 9.187Zm-13.98 0c0 2.68 1.61 5.184 4.79 5.184 3.18 0 4.79-2.501 4.79-5.184 0-2.645-1.859-5.22-4.79-5.22-3.145 0-4.79 2.575-4.79 5.22ZM75.45 0v9.76c1.036-1.822 3.933-2.823 5.79-2.823 5.147 0 8.974 3.146 8.974 9.223 0 5.79-3.897 9.222-9.079 9.222-2.144 0-4.324-.714-5.684-2.823l-.287 2.431h-4.076V0h4.363Zm.284 16.16c0 3.18 2.358 5.184 5.112 5.184 2.788 0 5.006-2.11 5.006-5.184 0-3.18-2.218-5.147-5.006-5.147-2.75-.003-5.112 2.072-5.112 5.147ZM96.699 19.232c1.013 1.95 3.333 2.782 6.223 1.73 1.51-.551 3.285-1.842 3.872-3.158l3.562 1.632c-1.086 2.45-3.623 4.362-6.143 5.28-5.71 2.078-10.389-.214-12.32-5.523-1.835-5.038.271-9.913 5.647-11.87 5.541-2.017 10.251.15 12.093 7.202l-12.934 4.707Zm7.768-6.67c-1.051-2.052-3.122-2.514-5.474-1.66-2.218.806-3.639 2.543-3.46 4.91l8.934-3.25Z"
                fill="#fff" />
            <path fill-rule="evenodd" clip-rule="evenodd"
                d="M39.763 11.99c-1.414-2.95-4.361-4.982-8.293-4.982-5.614 0-9.116 4.145-9.116 9.187 0 3.218 1.379 6.049 3.847 7.69.14-.106.225-.174.225-.174a62.402 62.402 0 0 0 3.6-2.534c-2.202-.648-3.313-2.755-3.313-4.982 0-2.645 1.647-5.22 4.79-5.22 2.766 0 4.576 2.294 4.771 4.775a62.21 62.21 0 0 0 3.489-3.76ZM31.06 25.374a62.346 62.346 0 0 0 9.605-8.41c-.332 4.716-3.731 8.418-9.16 8.418-.15 0-.298-.003-.445-.008Z"
                fill="#8DD3BB" />
            <path
                d="M43.318 4.538C38.863 2.9 35.66 6.424 35.66 6.424l3.036 1.766c1.138-.653 1.645-.02 1.775.328.093.245-.032.513-.13.66l-.74.933a62.282 62.282 0 0 1-13.915 12.41s-1.58 1.274-2.422 1.293c-.707.016-1.023-.58-.427-1.434l-1.466-3.308s-3.814 2.499-3.075 6.89c.313 1.855 2.055 3.174 3.907 2.84.946-.17 2.116-.632 3.546-1.578l2.633-1.72a62.325 62.325 0 0 0 13.915-12.435l.905-1.093c1.364-1.552 1.976-2.83 2.183-3.84.319-1.546-.596-3.056-2.068-3.598Z"
                fill="#8DD3BB" />
        </g>
        <defs>
            <clipPath id="clip0_108_21029">
                <path fill="#fff" d="M0 0h110.353v36H0z" />
            </clipPath>
        </defs>
    </svg>

    <div class="flex items-center">
        <div class="flex items-center justify-center w-32 h-24 mr-4 border-b-4 border-transparent">
            <a href="#" class="font-semibold text-white text-md font-montserrat">Login</a>
        </div>
        <div class="flex items-center justify-center w-32 h-24">
            <a href="#"
                class="px-4 py-2 font-semibold bg-white rounded-lg text-blackish-green text-md font-montserrat">Sign
                Up</a>
        </div>
    </div>
</nav>
